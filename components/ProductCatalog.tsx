          ...
          <div className="min-w-0">
            {sections.map((section) => {
              ...
              return <section key={section} id={categoryId(section)} className="scroll-mt-32 pb-9 last:pb-0 sm:scroll-mt-6 sm:pb-14"><div className="mb-4 border-b border-zinc-200 pb-3 sm:mb-6 sm:pb-4"><h2 className="text-2xl font-black sm:text-4xl">{section}</h2></div><div className="grid grid-cols-2 gap-3 sm:gap-6 xl:grid-cols-2">{sectionProducts.map((product) => {
                const discountedPrice = getDiscountedPrice(product); return <article key={product.id} className="flex min-w-0 flex-col rounded-2xl border border-zinc-200 bg-zinc-50 p-2.5 sm:rounded-3xl sm:p-4 sm:transition sm:hover:-translate-y-1 sm:hover:shadow-lg"><a href={getOfferPath(product)} className="block min-w-0"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-white p-1 sm:h-48 sm:aspect-auto sm:rounded-2xl sm:p-0">{product.image ? <img src={product.image} alt={product.name} loading="lazy" className="max-h-full max-w-full object-contain" /> : <span className="text-xs text-zinc-400 sm:text-base">Brak zdjęcia</span>}</div><h3 className="mt-2 line-clamp-3 min-h-[3.6rem] break-words text-[13px] font-bold leading-[1.2] sm:mt-4 sm:min-h-0 sm:text-base sm:leading-normal">{product.name}</h3></a><div className="mt-2 sm:mt-3"><p className="text-[11px] leading-tight text-zinc-400 line-through sm:text-sm">Allegro: {product.price} {product.currency}</p><p className="mt-1 text-base font-black leading-tight text-green-700 sm:text-xl">{discountedPrice} {product.currency}</p><p className="text-[11px] font-bold text-green-700 sm:text-sm">Cena Widia.tech</p></div><p className="mt-1.5 text-[11px] text-zinc-600 sm:mt-2 sm:text-sm">{product.stock > 0 ? `Dostępne: ${product.stock} szt.` : "Chwilowo niedostępne"}</p><div className="mt-auto grid gap-1.5 pt-3 sm:gap-2 sm:pt-4"><button type="button" disabled={product.stock <= 0} onClick={() => addToCart(product)} className="flex min-h-11 items-center justify-center rounded-xl bg-green-600 px-2 py-2 text-center text-[11px] font-black leading-tight text-white active:scale-[0.98] disabled:bg-zinc-400 sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">Dodaj do koszyka — zakup za pobraniem</button><a href={getOfferPath(product)} className="flex min-h-10 items-center justify-center rounded-xl bg-orange-500 px-2 py-2 text-center text-[11px] font-bold text-white active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">Kup na Allegro</a></div></article>;
              })}</div></section>;
            })}
